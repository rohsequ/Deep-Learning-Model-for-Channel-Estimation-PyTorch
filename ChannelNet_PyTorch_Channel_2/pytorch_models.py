...
class SRCNN(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.layer1 = torch.nn.Sequential(
            torch.nn.Conv2d(2, 64, kernel_size=9, stride=1, padding=4),
            torch.nn.ReLU(),
        ...
        # L3 ImgIn shape=(?, :, :, 32)
        # Conv ->(?, :, :, 1)
        self.layer3 = torch.nn.Conv2d(32, 2, kernel_size=5, stride=1, padding=2)
...
class DnCNN(torch.nn.Module):
    ...
    def __init__(self):
        ...
        # Conv -> (?, :, :, 64)
        self.layer1 = torch.nn.Sequential(
            torch.nn.Conv2d(2, 64, kernel_size=5, stride=1, padding=2),
            torch.nn.ReLU(),
            # torch.nn.MaxPool2d(kernel_size=2, stride=2),
            # torch.nn.Dropout(p=1 - keep_prob)
        )
        ...
        # L3 ImgIn shape=(?, :, :, 64)
        # Conv ->(?, :, :, 1)
        self.layer3 = torch.nn.Conv2d(64, 2, kernel_size=5, stride=1, padding=2)
        torch.nn.init.kaiming_normal_(self.layer3.weight, nonlinearity='relu')
        self.layer3.bias.data.fill_(0.01) 
...
def train_DnCNN(train_size, test_size, dataset, n_epochs, model_name, device, path_to_SRCNN, load_from_checkpoint=None):
    ...
    if model_name == "DnCNN":
        model = DnCNN()
        srcnn_model = SRCNN()
        srcnn_model.load_state_dict(torch.load(str(path_to_SRCNN)))
    else:
        raise Exception("The Correct Model name WASNT given, Please check..")
